...
class TestChatFTS:
    ...
    class TestTokenize:
        ...
        def test_lowercases_input(self) -> None:
            assert ChatFTS._tokenize("Hello WORLD") == ["hello", "world"]

        def test_strips_punctuation(self) -> None:
            assert ChatFTS._tokenize("hello, world!") == ["hello", "world"]

        def test_splits_on_whitespace(self) -> None:
            assert ChatFTS._tokenize("one two three") == ["one", "two", "three"]

        def test_returns_only_alphanumeric(self) -> None:
            assert ChatFTS._tokenize("foo@bar #baz 123") == ["foo", "bar", "baz", "123"]

        def test_empty_string_returns_empty_list(self) -> None:
            assert ChatFTS._tokenize("") == []

        def test_numbers_are_included(self) -> None:
            assert ChatFTS._tokenize("gpt4 model2024") == ["gpt4", "model2024"]

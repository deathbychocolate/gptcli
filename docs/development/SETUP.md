...
[5] Run `make setup`.
[6] Run `make install`.
[7] Run `gptcli` or `python3 gptcli/main.py`.
[8] Run `make test` and `make coverage`.
[9] Run [./gptcli/main.py](./gptcli/main.py) with VSCode's or PyCharm's debug feature.
